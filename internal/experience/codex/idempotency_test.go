...
func TestCursorCheckpoint_AcceptsTurnUUIDVariants(t *testing.T) {
	tests := []struct {
		...
		{name: "string", value: "turn-1", want: "turn-1"},
	}
...
func TestScan_CursorPersistsAcrossDatabaseRestart(t *testing.T) {
	...
	for i, envelope := range first.Envelopes {
		...
		result, err := service.Ingest(ctx, project.ID, &experience.IngestInput{
			Envelope:       envelope,
			SourceInstance: req.Instance.RolloutPath,
	...
	if err := storage.WithTx(ctx, db, func(tx *sql.Tx) error {
		cursor, err := storage.FindIngestionCursor(ctx, tx, project.ID, domain.SourceCodex, req.Instance.RolloutPath)
		if err != nil {
			return err
		}
		if cursor != nil {
...
func fixtureScanRequest(t *testing.T) ScanRequest {
	t.Helper()
	path, err := filepath.Abs("testdata/fixtures/rollout-001.jsonl")
	if err != nil {
...
func saveTestProject(t *testing.T, db *storage.DB, root string) *domain.Project {
	...
	project := &domain.Project{
		ID:            "codex-idempotency",
		ProjectKey:    "codex-idempotency",
		DisplayName:   "Codex Idempotency",
		CanonicalPath: root,
		Fingerprint:   "codex-idempotency-fixture",
		CreatedAt:     now,
		UpdatedAt:     now,
	}
