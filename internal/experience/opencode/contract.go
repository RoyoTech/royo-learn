...
const SchemaTag = "opencode/sqlite-v1"

// SourceInstance is one located OpenCode session store that an adapter can
// scan. Discover returns zero or more of these per call.
type SourceInstance struct {
	// Source is always domain.SourceOpenCode for this adapter.
	...
	ProjectRoot string
	// DBPath is the canonical absolute path of the OpenCode SQLite database
	// file. The adapter only opens it in read-only mode.
	DBPath string
...
type ScanRequest struct {
	...
	// TailQuietPeriod bounds the time the adapter waits for an in-flight turn
	// to settle before treating it as stable. Zero falls back to the contract
	// default.
	TailQuietPeriod time.Duration
	// TailClock is injectable for tests. Nil falls back to time.Now.
	TailClock func() time.Time
}
...
type TraceResult struct {
	...
	Redacted bool
	// SourceChanged reports whether the source DB has been mutated since the
	// referenced turn was ingested. When true, the caller should treat the
	// excerpt as advisory only.
	SourceChanged bool
